...
void remove(Node *&head, int oldKey)
{
    ...
    Node* current = head;
        while (current->next != NULL && current->next->key != oldKey) {
            current = current->next;
        }
...
void insert_after(Node *head, int oldKey, int newKey)
{
    for (Node *curr = head; curr != NULL; curr = curr->next)
    {
        ...
        if (curr->key == oldKey) {
            Node* p1 = curr->next;
            Node* newnode = new Node(newKey, p1);
            curr->next = newnode;
            break;
        }
    }
...
Node* interleave(Node* list1, Node* list2) {
    ...
    while (current1 != NULL || current2 != NULL) {
        if (current1 != NULL) {
            if (head == NULL) {
                newLinked = new Node(current1->key, NULL);
                head = newLinked;
            } else {
                newLinked->next = new Node(current1->key, NULL);
        ...
        if (current2 != NULL) {
            if (head == NULL) {
                newLinked = new Node(current2->key, NULL);
                head = newLinked;
            } else {
                newLinked->next = new Node(current2->key, NULL);
                newLinked = newLinked->next;
            }
