...
{
    bool wordContainsTest = true;
    bool testContainsWord = true;
    bool atEqualLength = true;

    for (size_t i=0; i<word.length(); i++) {
        if (test.find(word[i]) == std::string::npos) {
            testContainsWord = false;
        }
    }

    for (size_t j=0; j<test.length(); j++) {
        if (test.find(word[j]) == std::string::npos) {
            wordContainsTest = false;
        }
    }

    if (test.length() != word.length()) atEqualLength = false;
    return (wordContainsTest && testContainsWord && atEqualLength);
}
