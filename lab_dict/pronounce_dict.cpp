...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    if (dict.count(upperWord1) && dict.count(upperWord2)) {


        const vector<string>& pronun1 = dict.at(upperWord1);
        const vector<string>& pronun2 = dict.at(upperWord2);


        if (pronun1 == pronun2) {
            return true; // The words are homophones
        }
    }

    return false; // The words are not homophones or not in the dictionary
}
